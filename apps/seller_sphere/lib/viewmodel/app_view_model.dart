...
class AppViewModel extends ChangeNotifier {
  ...
  Future<void> updateTodayTarget(double amount) async {
    ...
    _todayTarget = target;
    triggerNotification(
      "Target Penjualan Diperbarui",
      "Target hari ini diatur sebesar ${formatRupiah(amount)}",
    );
    notifyListeners();
  }
  ...
  void addToCart(Product product) {
    if (product.stock <= 0) {
      triggerNotification(
        "Stok Habis",
        "Produk ${product.name} tidak memiliki stok tersisa.",
      );
      return;
    }
    ...
    if (currentQty + 1 > product.stock) {
      triggerNotification(
        "Stok Tidak Cukup",
        "Hanya tersedia ${product.stock} unit untuk ${product.name}.",
      );
      return;
    }
  ...
  Future<void> _checkTargetMilestoneAchievement(double newSaleAmount) async {
    ...
    final double previousSales = await getTodaySalesTotal();
    ...
    if (prevPercentage < 50 && newPercentage >= 50) {
      triggerNotification(
        ...
        "Mantap! Anda sudah mencapai setengah target penjualan hari ini.",
      );
    } else if (prevPercentage < 100 && newPercentage >= 100) {
      triggerNotification(
        "Target Harian 100% Tercapai! 🏆🔥",
        "Luar biasa! Target penjualan harian sebesar ${formatRupiah(target)} telah terpenuhi!",
      );
    }
  }
  ...
  Future<double> getTodaySalesTotal() async {
    ...
    final todayTrans = _latestTransactions.where(
      (t) => t.timestamp >= startOfToday && t.timestamp <= endOfToday,
    );
  ...
  Future<void> addProduct({
    ...
    triggerNotification(
      "Produk Ditambahkan",
      "Produk $name berhasil dimasukkan ke inventaris.",
    );
  }
  ...
  Future<void> updateProduct(Product product) async {
    ...
    triggerNotification(
      "Produk Diperbarui",
      "Data ${product.name} telah disimpan.",
    );
  }
  ...
  Future<void> simulatePrintLabel() async {
    ...
    triggerNotification(
      "Cetak Selesai 🖨️",
      "Label untuk ${_selectedProductForLabel!.name} sukses dikirim ke printer.",
    );
    notifyListeners();
  }
  ...
  void generateNewSyncCode() {
    ...
    final random = Random();
    final code =
        "SPHERE-${String.fromCharCodes(Iterable.generate(4, (_) => chars.codeUnitAt(random.nextInt(chars.length))))}-${String.fromCharCodes(Iterable.generate(4, (_) => chars.codeUnitAt(random.nextInt(chars.length))))}";
    _syncCode = code;
  ...
  Future<void> triggerManualSync() async {
    ...
    triggerNotification(
      ...
      "Data inventaris dan penjualan berhasil disinkronkan.",
    );
    notifyListeners();
  }
  ...
  Future<String> exportProductsToCsv() async {
    ...
    sb.writeln(
      "ID,Nama Produk,SKU,Stok,Harga Beli,Harga Jual,Kategori,Ambang Minimum",
    );
    for (var p in prods) {
      final name = p.name.replaceAll('"', '""');
      final category = p.category.replaceAll('"', '""');
      sb.writeln(
        '${p.id},"$name",${p.sku},${p.stock},${p.purchasePrice},${p.sellingPrice},"$category",${p.minStockThreshold}',
      );
    }
    return sb.toString();
  }
  ...
  Future<String> exportTransactionsToCsv() async {
    ...
    for (var t in trans) {
      ...
      sb.writeln(
        "${t.id},$dateStr,${t.totalAmount},${t.totalProfit},${t.paymentMethod}",
      );
    }
    return sb.toString();
  }
  ...
  Future<bool> importProductsFromCsv(String csvContent) async {
    try {
      ...
      for (var i = 1; i < lines.length; i++) {
        ...
        if (tokens.length >= 7) {
          final p = Product(
            ...
            category: tokens[6],
            minStockThreshold: (tokens.length > 7)
                ? (int.tryParse(tokens[7]) ?? 5)
                : 5,
          );
      ...
      triggerNotification(
        "Impor Sukses",
        "Data produk berhasil diimpor dari file CSV.",
      );
      return true;
    } catch (e) {
      triggerNotification(
  ...
  Stream<NotificationItem> get notificationFlow =>
  ...
  Future<void> _seedInitialData() async {
    ...
    if (prodCount == 0) {
      ...
      for (var i = 0; i <= 6; i++) {
        ...
        await _repository.insertTarget(
          SalesTarget(dateString: dateStr, targetAmount: 1000000.0),
        );
