...
import 'package:seller_sphere/utils/dashboard_helpers.dart';
import 'order_pickup_item.dart';
...
class WeeklyOrderChart extends StatefulWidget {
  ...
  const WeeklyOrderChart({super.key, required this.orders, required this.onNavigateToChat});
...
class _WeeklyOrderChartState extends State<WeeklyOrderChart> {
  ...
  List<Triple<String, String, DayOrderStats>> _getDaysData() {
    ...
    for (int i = 0; i <= 6; i++) {
      ...
      final completed = dayOrders.where((o) => o.status == "Selesai Diambil").length;
  ...
  Widget build(BuildContext context) {
    ...
    final maxOrders = daysData.map((d) => d.third.total).reduce((a, b) => a > b ? a : b).clamp(5, 1000).toDouble();
    final selectedDayStats = daysData[_selectedIndex].third;
            ...
            // Chart Legend
             Row(
                ...
                children: [
                  Row(
                    children: [
                       _LegendItem(color: Color(0xFF4CAF50), text: "Selesai Diambil"),
                      const SizedBox(width: 12),
                      _LegendItem(color: Color(0xFFFFA500), text: "Belum Diambil"),
                    ],
                  ),
                  Text(
                     ...
                     return BarChartGroupData(
                       ...
                       barRods: [
                         BarChartRodData(
                           ...
                           rodStackItems: [
                              BarChartRodStackItem(0, stats.completed.toDouble(), isSelected ? Color(0xFF4CAF50) : Color(0xFF4CAF50).withValues(alpha: 0.7)),
                              BarChartRodStackItem(stats.completed.toDouble(), stats.total.toDouble(), isSelected ? Color(0xFFFFA500) : Color(0xFFFFA500).withValues(alpha: 0.7)),
                           ]
                         )
                       ]
                     );
                   ...
                   child: Column(
                    children: [
                      ...
                      Text(
                        daysData[i].second,
                        style: TextStyle(
                          color: isSelected ? Theme.of(context).primaryColor.withValues(alpha: 0.8) : Theme.of(context).colorScheme.onSurfaceVariant.withValues(alpha: 0.5),
                          fontSize: 9,
                        ),
                      ),
                    ],
              ...
              Card(
                 ...
                 child: const Center(
                   child: Padding(
                     ...
                     child: Text("Tidak ada orderan masuk untuk tanggal ini.", style: TextStyle(fontSize: 12)),
                   ),
                 ),
              )
            else
